...
function disableElement(){
	...
	setTimeout(disableElement, <%=size*6000%>);
</script>
...
function startTime() {
    ...
    document.getElementById('txt').innerHTML ="Remaining time is "+i+" minutes";
...
<body onload="startTime()">
					...
					<div class="row" style="padding-top: 5px;">
						...
						<div class="col-xs-12 col-sm-5">
							<span class="text-primary myh1">JNTUA</span><span
								...
								class="text-primary myh1">ChatBot</span>
						</div>
	...
	<div class="container">
		...
		<form action="submitexam.htm" method="post">
		<% int count=0; %>
				...
				<div class="row" id="div<%=++count%>">
					<div
						...
						<div class="row">
							<div
								class="col-lg-6 col-lg-offset-3 col-sm-6 col-sm-offset-3 col-xs-12 ">
								<strong> <%=count%>)&nbsp;${ques.quationTitle }</strong>
								<input type="hidden" name="q<%=count%>" value="${ques.qid }"/>
								<input type="hidden" name="qtype<%=count%>" value="${ques.quetionType }"/>

							</div>

						</div>
						<c:choose>
						<c:when test='${ques.quetionType eq "single" or ques.quetionType eq "tf"  }'>
			...
			<div class="row">
				<div
					...
					<input type="button" class="btn btn-success "  onclick="showDiv(currentDiv + 1)" id="nxt" style="width: 16%;" value="Next">
					<input type="hidden" name="sid" value="${requestScope.subject.subjectId }">
					
					<input type="button" class="btn btn-success "  onclick="showDiv(currentDiv - 1)" id="prv" style="width: 16%;" value="Previous"/>
					<input type="submit" class="btn btn-success " value="Submit" id="btnExamSubmit" style="width: 15%;">
